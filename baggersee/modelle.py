...
class Tagesdatensatz:
    ...
    einnahmen: float
    oeffnung_von: str  # Format "HH:MM"
    oeffnung_bis: str  # Format "HH:MM"


def ist_unvollstaendig(e: Tagesdatensatz) -> bool:
    """
    Prüft, ob für einen Tag Kernangaben fehlen: Temperatur nicht erfasst
    (dargestellt als NaN, z.B. aus einem Import ohne Messwert), keine
    Öffnungszeiten hinterlegt, oder Einnahmen trotz Besuchern bei 0€.
    """
    if math.isnan(e.wassertemperatur) or math.isnan(e.lufttemperatur):
        return True
    if not e.oeffnung_von or not e.oeffnung_bis:
        return True
...
def validiere_oeffnungszeiten(von: str, bis: str) -> None:
